import { BPEdge, BPNode, PinDefinition, PinType } from "../types";
...
const classFromTemplate = (node: BPNode): string => {
  ...
  if (key) {
    try {
      ...
      // ignore unknown template errors
    }
  }
...
const makeNode = (node: BPNode, index: number): SerializableNode => {
  ...
  const pushPins = (pinDefs: PinDefinition[] = [], direction: "EGPD_Input" | "EGPD_Output") => {
  ...
  pushPins(node.data.inputs as PinDefinition[], "EGPD_Input");
  pushPins(node.data.outputs as PinDefinition[], "EGPD_Output");
...
export const buildUeClipboard = (nodes: BPNode[], edges: BPEdge[]): string => {
  ...
  serializableNodes.forEach((node) => {
    lines.push(
      `Begin Object Class="${node.classPath}" Name="${node.name}"`
    );
